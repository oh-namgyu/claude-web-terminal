...
function listResumableSessions() {
    ...
    // Track the transcript files seen this pass so we can drop cache entries
    // for deleted transcripts afterwards (otherwise _transcriptCache grows
    // unbounded, holding previews for sessions that no longer exist on disk).
    ...
    // Prune cache entries whose transcript file is gone (deleted session).
    for (const key of _transcriptCache.keys()) {
        if (!seenPaths.has(key)) _transcriptCache.delete(key);
    }
    return all;
}
