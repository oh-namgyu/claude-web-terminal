...
// Transcript previews, keyed by file path and invalidated on mtime. Bounded so
// a long-running server doesn't accumulate an entry per session it has ever
// seen (the listing only ever asks about live sessions, but "live" churns).
const _transcriptCache = new Map();
const TRANSCRIPT_CACHE_MAX = 200;

// Session-id validators, shared between server.js and this module so the
// accepted shapes stay in lockstep.
//   - JOB id      (background `--bg` session): hex, ≥6 chars
//   - METADATA id (bg jobId OR full session UUID with dashes): the above OR a
//     36-char dashed UUID
const JOB_ID_RE = /^[a-f0-9]{6,}$/i;
const METADATA_ID_RE = /^[a-f0-9-]{36}$|^[a-f0-9]{6,}$/i;
function isValidJobId(id) { return typeof id === 'string' && JOB_ID_RE.test(id); }
function isValidMetadataId(id) { return typeof id === 'string' && METADATA_ID_RE.test(id); }
...
function _readTranscriptPreview(filepath) {
    try {
        ...
        // recently inserted — good enough eviction for a cache this small.
        if (_transcriptCache.size > TRANSCRIPT_CACHE_MAX) {
            _transcriptCache.delete(_transcriptCache.keys().next().value);
        }
        return info;
    } catch {
        return null;
...
function stopSession(id, env, claudeBin = 'claude') {
    return new Promise((resolve, reject) => {
        if (!isValidJobId(id)) return reject(new Error('invalid session id'));
...
function _saveMetadata(meta) {
    ...
    // Write to a unique temp file in the same dir, then rename over the target.
    // rename(2) is atomic on POSIX, so a concurrent reader/writer never sees a
    // half-written or truncated metadata file (the previous in-place write
    // could lose updates or corrupt the JSON under contention).
    const tmp = `${METADATA_FILE}.${process.pid}.${Date.now()}.tmp`;
    try {
...
// Listing past (resumable) transcripts is NOT here — that is the local session
// browser in lib/resume-sessions.js, which is the single source of truth for
// "what can `claude --resume` reopen on this machine". This module only deals
// with sessions that are currently alive.
module.exports = {
    listSessions, createSession, stopSession,
    updateSessionMetadata, getSessionMetadata,
    isValidJobId, isValidMetadataId,
